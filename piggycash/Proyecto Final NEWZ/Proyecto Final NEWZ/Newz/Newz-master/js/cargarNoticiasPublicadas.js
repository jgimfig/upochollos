 ...
 */
var EJECUTA_NOTICIAS_PUBLICADAS = true;
var DETECTAR_SCROLL_NOTICIAS_PUBLICADAS = true;
...
$(document).ready(function () {
    ...
    $(window).scroll(function () {
        if (DETECTAR_SCROLL_NOTICIAS_PUBLICADAS) {
            var docViewTop = $(window).scrollTop();
            var docViewBottom = docViewTop + $(window).height();

            var elemTop = $("#cargarNoticias").offset().top;
            var elemBottom = elemTop + $("#cargarNoticias").height();

            if ((elemBottom <= docViewBottom) && (elemTop >= docViewTop)) {
                if (EJECUTA_NOTICIAS_PUBLICADAS) {
                    getNoticiasServidor();
                    EJECUTA_NOTICIAS_PUBLICADAS = false;
                }
            } else {
                EJECUTA_NOTICIAS_PUBLICADAS = true;
            }
        }
    });
});
...
function getNoticiasServidor() {
    var noticiasCargadas = parseInt($("#cargarNoticias").find("figure")[0].id);

    $.ajax({
        url: 'noticiasPublicadas.php',
        ...
        success: function (respuestas_json) {
            ...
            if (noticias.length > 0) {
                ...
                $("#cargarNoticias").find("figure")[0].id = noticiasCargadas + noticias.length;

            } else {
                $("#cargarNoticias").remove();
                EJECUTA_NOTICIAS_PUBLICADAS = false;
                DETECTAR_SCROLL_NOTICIAS_PUBLICADAS = false;
            }
        }
    });
}
...
function cargarNoticias(noticia) {
    ...
    $("#cargarNoticias").before(stringNoticia);
}
