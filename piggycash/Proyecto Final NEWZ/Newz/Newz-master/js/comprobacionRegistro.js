...
function comprobarRegistro() {
    ...
    if (!validarUsuario.test($("#usuarioTextField").val())) {
        siiimpleToast.alert('Usuario inválido');
    ...
    if ($("#contrasenaTextField").val().length <= 0) {
        siiimpleToast.alert('Contraseña inválida');
    ...
    if (!validarEmail($("#emailTextField").val())) {
        siiimpleToast.alert('Email inválido');
...
function usuarioExiste(){
     siiimpleToast.alert('El usuario ya está registrado');
}